$(document).ready(function(){
    ...
    if (password.length == 0 ){
          $('.form-group:first').append('<span class="erase" >Por favor escribe una contraseña</span>');      
        }else if (password.length < 6){
          $('.form-group:first').append('<span class="erase" >Contraseña muy corta, minimo 6 caracteres</span>');  
        }else if(password== 123456 || password== 098754){
          $('.form-group:first').append('<span class="erase" >Esta contraeña no es segura</span>');
    } 
    if( biciTypes== null || biciTypes== 0 ){
          $('.form-group:eq(1)').append('<span class="erase" >Escoge tu tipo de bici!</span>')};  
    }